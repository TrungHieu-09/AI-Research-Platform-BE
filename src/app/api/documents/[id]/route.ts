...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  try {
    ...
    const doc = await getDocumentById(resolvedParams.id, userId, role)
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> | { id: string } }) {
  try {
    ...
    const role = req.headers.get("x-user-role")!
    const resolvedParams = await params;
    await softDeleteDocument(resolvedParams.id, userId, role)
    return NextResponse.json({ message: "Document deleted successfully." })
  } catch (err: any) {
    const status = err.message === "Document not found." ? 404 : (err.message.includes("Permission denied") ? 403 : 500)
    return NextResponse.json({ error: err.message }, { status })
  }
}
