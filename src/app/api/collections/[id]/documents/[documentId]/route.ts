 ...
 */
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string; documentId: string }> }) {
  try {
    const { id, documentId } = await Promise.resolve(context.params);
